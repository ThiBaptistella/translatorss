...
(function($) {
	...
	$.fn.fitVids = function(options) {
		...
		if (!document.getElementById('fit-vids-style')) {
			...
			div.innerHTML = '<p>x</p><style id="fit-vids-style">' + css
					+ '</style>';
		...
		return this.each(function() {
			...
			$allVideos = $allVideos.not("object object"); // SwfObj conflict
															// patch
			...
			$allVideos.each(function() {
				...
				var height = (this.tagName.toLowerCase() === 'object' || ($this
						.attr('height') && !isNaN(parseInt(
						$this.attr('height'), 10)))) ? parseInt($this
								.attr('height'), 10) : $this.height(), width = !isNaN(parseInt(
						$this.attr('width'), 10)) ? parseInt($this
								.attr('width'), 10) : $this.width(), aspectRatio = height
						/ width;
				if (!$this.attr('id')) {
				...
				$this.wrap('<div class="fluid-width-video-wrapper"></div>')
						.parent('.fluid-width-video-wrapper').css(
