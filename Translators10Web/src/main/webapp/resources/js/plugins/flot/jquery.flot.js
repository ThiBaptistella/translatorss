 ...
 * Examples:
 * 
 * $.color.parse("#fff").scale('rgb', 0.25).add('a', -0.5).toString() var c =
 * $.color.extract($("#mydiv"), 'background-color'); console.log(c.r, c.g, c.b,
 * c.a); $.color.make(100, 50, 25, 0.4).toString() // returns
 * "rgba(100,50,25,0.4)"
 * 
 * Note that .scale() and .add() return the same modified object instead of
 * making a new one.
 * 
 * V. 1.1: Fix error handling so e.g. parsing an empty string does produce a
 * color rather than just crashing.
 */
(function(B) {
	B.color = {};
...
(function($) {
	function Plot(placeholder, data_, options_, plugins) {
		...
		function measureTickLabels(axis) {
			...
			function makeDummyDiv(labels, width) {
				return $('<div style="position:absolute;top:-10000px;' + width
						...
						+ axis.direction + 'Axis ' + axis.direction + axis.n
						+ 'Axis">' + labels.join("") + '</div></div>')
		...
		function allocateAxisBoxFirstPhase(axis) {
			...
			var lw = axis.labelWidth, lh = axis.labelHeight, pos = axis.options.position, tickLength = axis.options.tickLength, axismargin = options.grid.axisMargin, padding = options.grid.labelMargin, all = axis.direction == "x"
					? xaxes
					: yaxes, index;
		...
		function setupGrid() {
			...
			$.each(axes, function(_, axis) {
						...
						if (axis.show == null)
							axis.show = axis.used; // by default an axis is
													// visible if it's got data

						axis.reserveSpace = axis.show
								|| axis.options.reserveSpace;
		...
		function setupTickGeneration(axis) {
			...
			if (opts.mode == "time") {
				...
				for (var i = 0; i < spec.length - 1; ++i)
					if (delta < (spec[i][0] * timeUnitSize[spec[i][1]] + spec[i
							+ 1][0]
		...
		function drawSeriesBars(series) {
			...
			ctx.strokeStyle = series.color;
			var barLeft = series.bars.align == "left"
					? 0
		...
		function findNearbyItem(mouseX, mouseY, seriesFilter) {
			...
			for (i = series.length - 1; i >= 0; --i) {
				...
				if (s.bars.show && !item) { // no other point can be nearby
					var barLeft = s.bars.align == "left" ? 0 : -s.bars.barWidth
							/ 2, barRight = barLeft + s.bars.barWidth;
