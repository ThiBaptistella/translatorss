...
@Service
@Transactional
public class AmazonFilePhotoServiceImpl implements AmazonFilePhotoService{
    ...
    @Autowired
    private AmazonFilePhotoDao dao;
    
    @Inject
    public AmazonFilePhotoServiceImpl(@Value("${aws.s3.bucket}") String bucketName, AmazonS3 amazonS3) {
	...
	@Override
    public AmazonFilePhoto savePhoto(User user, String fileName, InputStream is, String contentType) {
	     FileType type = FileType.PHOTO;
	     ...
	     AmazonFilePhoto photoView = getAmazonFilePhotoByUserId(user);
	     if(photoView ==null){
		     AmazonFilePhoto amazonFile = new AmazonFilePhoto(key, url,new Date(),user, fileName, type);
		     return dao.savePhoto(amazonFile);
	     }else{
	    	 dao.remove(photoView);
	    	 AmazonFilePhoto amazonFile = new AmazonFilePhoto(key, url,new Date(),user, fileName, type);
		     return dao.savePhoto(amazonFile);
	     }
	...
	public AmazonFilePhoto getAmazonFilePhotoByUserId(User user) {
		return this.dao.getAmazonFilePhotoByUserId(user);
	}
}
